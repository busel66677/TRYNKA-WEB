...
async function currentRoomId(){
  const {data:{user}}=await botSb.auth.getUser();
  if(!user)return null;
  const title=document.getElementById('roomTitle')?.textContent?.trim();
  const {data:mine}=await botSb.from('room_players').select('room_id').eq('user_id',user.id);
  const ids=[...new Set((mine||[]).map(x=>x.room_id))];
  if(!ids.length)return null;
  const {data:rooms}=await botSb.from('rooms').select('id,name,created_at').in('id',ids).order('created_at',{ascending:false});
  return ((rooms||[]).find(r=>r.name===title)||(rooms||[])[0])?.id||null;
}
...
async function ensureBot(){
  ...
  try{
    ...
    alert('BOT: '+(e?.message||e));
    if(btn){btn.disabled=false;btn.textContent=old||'＋ BOT'}
...
async function botTick(){
  try{
    ...
    const room=await currentRoomId(); if(!room)return;
    const {data:gr}=await botSb.from('game_rounds').select('id,status,turn_user_id,turn_started_at').eq('room_id',room).eq('status','playing').order('id',{ascending:false}).limit(1).maybeSingle();
    if(!gr?.turn_user_id)return;
    const {data:p}=await botSb.from('profiles').select('is_bot').eq('id',gr.turn_user_id).maybeSingle();
    if(!p?.is_bot)return;
    const key=gr.id+':'+gr.turn_started_at;
    if(lastBotRound===key)return;
    lastBotRound=key;
    await sleep(1600+Math.floor(Math.random()*1400));
    const {error}=await botSb.rpc('bot_take_turn',{p_room:room});
    if(error){console.error('BOT turn error',error);lastBotRound=null}
  }catch(e){console.error('BOT tick',e)}
}
